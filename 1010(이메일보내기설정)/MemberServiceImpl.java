...
public class MemberServiceImpl implements MemberService {
	...
	public void send_mail(Member member, String div) throws Exception {
		...
		//회원가입 메일 내용
		if(div.equals("join")) {
			subject = "불밤야밤 Camping 회원가입 인증 메일입니다!"; //메일 제목 , msg-> 본문내용
		...
		try {
			HtmlEmail email = new HtmlEmail(); // HtmlEmail 클래스 : 이메일 컨텐츠를 HTML 형식으로 전송
			email.setDebug(true);
			email.setCharset(charSet);		// 한글 인코딩 설정
			email.setSSL(true);
			email.setHostName(hostSMTP);	// 네이버 메일 - smtp.naver.com
			email.setSmtpPort(587);		// SMTP 서버 포트 : 기본포트 465(SSL) 또는 네이버이용시 587(TLS)

			email.setAuthentication(hostSMTPid, hostSMTPpwd); // SMTP 서버 계정정보 : 로그인할 아이디, 패스워드
			email.setTLS(true);
			email.addTo(mail, member.getName(), charSet); // 받는 사람의 Email, 이름, 인코딩형식 설정
			email.setFrom(fromEmail, fromName, charSet); // 보내는 사람의 Email, 이름, 인코딩형식 설정
			email.setSubject(subject);	// 제목 설정
			email.setHtmlMsg(msg);	// 본문 설정
			email.send();			// 메일 전송
		} catch (Exception e) {
